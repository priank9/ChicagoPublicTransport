...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        #Initialize topic and parameters
        topic_name = f"cta.station.arrival.events.{station_name}" 
        super().__init__(
            topic_name,
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        try:
            logger.info(f"Event - station_id:{self.station_id} - train_id:{train.train_id} - direction:{direction} - line:{self.color} - train_status:{train.status.name} - prev_station_id:{prev_station_id} - prev-direction:{prev_direction}")
            ...
            logger.info(f"Unable to send arrival event at {self.topic_name}  - {e}")
