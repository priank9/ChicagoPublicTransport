...
class Line:
    ...
    def _handle_station(self, value):
        ...
        if value["line"].lower() != self.color.lower():
            print(f"_handle_station - value[line] - {self.color}")
            return
    ...
    def _handle_arrival(self, message):
        ...
        logger.info(f"_handle_arrival - prev_station_id - {prev_station_id}, prev_dir - {prev_dir}, Condition - {prev_dir is not None and prev_station_id is not None}")
        ...
        if station is None:
            ...
            return
        logger.info('----- Entering handle arrival -------')
